...
**Today membership (chunk 41 merge; persistence decided chunk 43):** the dashboard's Today list membership is client-local, not synced — no table, no `tasks` column; it lives in the Dashboard's `useTodayPlan` state (see §13). Chunk 43 resolved the persistence deferral **per-device, in localStorage, not the DB**: only the manual deltas persist (`hup:todayPlan` = `{ date, pinned[], removed[] }`), day-scoped to the local-midnight frame `lib/today` uses — a stored entry dated before today is discarded on read, and delta ids whose task no longer exists are pruned on the id-set reconcile. The resolved set is always re-derived as `(autoTodayIds(tasks) ∪ pinned) − removed`, so auto signals keep flowing as tasks change.

**Scheduled blocks (chunk 37).** A task's slot on the Week Planner: one block per task (`unique (task_id)` — the unscheduled tray is exactly "open tasks with no block"), deleted with its task. **Done has one source of truth: `tasks.completed_at`** (chunk 37 revisions). The planner's done toggle is a single `tasks.markComplete` write; rendering derives done from the task only (`blockIsDone(task)`), so completing or un-completing a task on any surface — Dashboard, planner, bulk complete, import, AI triage — shows correctly on the planner on the next realtime refresh. The `scheduled_blocks.done` mirror column and its migration-11 triggers were dropped in chunk 39 (migration 12); there is no block-level done anywhere. Chunk 38's carryover consumes the same task-derived flag. **Apple Calendar mirror (chunk 39).** With `settings.planner_writeout` on (opt-in, Settings → Calendars; only while `caldav_status = 'ok'`), every block write is mirrored to the selected iCloud calendar *after* its Supabase write — `scheduled_blocks` is the source of truth and the event is best-effort: a calendar failure toasts once and never rolls the block back, nothing calendar-related enters the outbox, and offline placements simply leave `calendar_uid` null. `calendar_uid` holds the event's uid (`hupo-block-<uuid>`, tagged by the proxy so `/busy` excludes it and reports it as `plannerEvents` instead — the planner therefore never counts a block twice). Once per week load — deduped on `weekKey:busyRefreshKey` **plus a content signature of the visible week's blocks** (id + startAt + endAt + calendarUid, volatile fields excluded; chunk 43, F2) — the planner reconciles the visible week against `plannerEvents`: events no block claims are deleted (task delete/cascade, wipe, replace-import), blocks with a null uid are backfilled, and blocks whose event drifted ≥1 minute are rewritten. The signature makes the dedup self-invalidating: drift written after a week already reconciled (another device's write, an offline outbox drain, a failed mirror write) is repaired on that week's next blocks load with no `busyRefreshKey` bump, while an unchanged week still never re-reconciles. The signature compares timestamps as **instants**, not strings (chunk 48, D13) — an optimistic patch writes `…T20:00:00.000Z` and the saved row comes back as PostgREST's `…T20:00:00+00:00`, so before this every block save looked like new content and re-ran the whole reconcile. The dedup store keeps one entry (the last signature) per `weekKey:busyRefreshKey`, so it stays bounded. The drift comparison reads `plannerEvents` from the last `/busy` **plus a session record of the events the mirror itself wrote** (uid → the start/end it wrote; chunk 46) — the busy fetch has no dependency on blocks or mirror writes, so without the record a block mirrored since that fetch is invisible to the drift loop and silently never repaired. Where only one of the two knows a uid it decides; where both do, the drift comparison takes **whichever observed the event more recently** (chunk 47) — the `/busy` snapshot when it was fetched after the mirror's write, the record otherwise. So an event edited in Calendar.app and reported by a later `/busy` is still seen as drift and still rewritten to the DB time — one-way as ever — while a snapshot older than the mirror's own write no longer makes the mirror rewrite what it just wrote, which is what made a single drag-move fire two identical PATCHes. The backfill and drift loops additionally skip any block whose mirror write the planner is mid-way through issuing (chunk 48, D12): a move/resize/carryover patches its block optimistically and only mirrors after the Supabase write resolves, so without this the reconcile fired on the optimistic frame and wrote the very times the mirror was about to write. The mark is set synchronously before the optimistic patch and released when the hook settles; a pass that deferred a block releases its dedup key, so a mirror write that never landed is still repaired on the next blocks change. The orphan sweep deliberately reads `plannerEvents` alone (it deletes what the calendar actually reports), and skips uids the mirror itself deleted — `afterDelete`'s deletes and, since chunk 49 (D15), the sweep's own successful ones. So an unschedule's delete is never repeated by the reconcile that follows its blocks refetch, and an orphan is never re-deleted by a pass that re-runs because a previous one deferred a pending block: D12's key release lets the whole pass run again with identical arguments, and before D15 the sweep recorded nothing, so a stray planner event in the snapshot was deleted once per deferred pass. A sweep delete that *failed* records nothing, so the uid stays sweepable and the next pass retries — the rule D8 already applies to `afterDelete`'s failure path. Only the sweep reads that set, so it reaches neither the drift loop nor the backfill. The reconcile is additionally gated on the busy and blocks state carrying the visible week's identity (chunk 40), so a week-change transition frame can never consume the reconcile key against stale data. Title drift is not reconciled (`plannerEvents` carries no summary) and task-title edits are not propagated live. One-way only: edits made in Calendar.app never flow back. Turning the toggle off or disconnecting Apple Calendar leaves existing events in place. **Fill-my-week proposals (chunk 38) are client-only and ephemeral:** a `useState` snapshot of the visible week's occupancy that is never written to Supabase/Dexie, never counted toward capacity, and dropped by any block mutation, drag activation or week change — only `Place all` writes, one sequential `scheduled_blocks` insert per proposal. **Carryover moves scan the current week:** a past, unfinished block's "Move to next open slot" always targets today → Sunday of the week containing today (fetching that week's busy + blocks on demand when a past week is visible), never the visible week. All planner math is browser-local (instants ↔ local day/minute on the client; `settings.timezone` is never consulted). Overlap with busy is advisory — surfaced in the drop preview and the toast, never blocked. Last write wins on `updated_at`. **Export/import (chunk 50).** `scheduled_blocks` round-trips through the Settings export/import payload. The export is a faithful dump — `calendar_uid` included, not redacted — but **import writes `calendar_uid: null` on every incoming block row**: a re-imported uid names a VEVENT the file cannot vouch for (deleted by hand, a different calendar selected since, written by another device), and a uid that appears in neither `plannerEvents` nor the mirror's session record is a block the drift loop finds no observation for and skips forever — mirrored to nothing. Null instead re-uses the backfill path above: the next load of that block's week creates the event and stamps a fresh uid (with `planner_writeout` off at import time, blocks simply stay uid-less until it is turned on — chunk-39 behaviour), and the orphan sweep removes whatever stale event the file's uid used to name, per week, as those weeks are visited. The key is **optional on read** (absent → `[]`) and `EXPORT_VERSION` stays `1`, so every export file already on disk still imports. Replace-import deletes blocks explicitly before `tasks` (which would cascade them anyway — the list states the intent) and re-inserts them after `tasks`; Merge upserts on `task_id`, the table's real invariant, so a file whose block targets a task that already has a different block resolves to one block rather than violating `unique (task_id)` partway through. Because the conflict update writes every column, a live block matched on `task_id` has its `calendar_uid` nulled as well, so a Merge re-mirrors those blocks on the next load of their week. "Wipe my data" still clears blocks through the `tasks` cascade.
...
**Discovery keeps subscribed calendars; every calendar has its own color (chunk 51b).** Discovery (`test-credentials`, `GET /api/calendar/calendars`) runs the proxy's own Depth-1 PROPFIND on the calendar home rather than tsdav's `fetchCalendars`, which keeps only `<calendar/>` collections — iCloud reports every *subscribed* calendar (US Holidays, any webcal feed) as `<cs:subscribed/>`, so chunk 51 as shipped never listed them. Kept: `calendar` and `subscribed` collections whose component set is empty or includes `VEVENT`; excluded: VTODO-only collections (Reminders) and the schedule-inbox / outbox / notification collections. **Not reachable by design:** Birthdays, Siri Suggestions, "Scheduled Reminders" and any non-iCloud account (Google, Exchange, On My Mac) do not exist on iCloud CalDAV — a work calendar comes in through the Outlook ICS feed. Each discovered calendar carries iCloud's `calendar-color` normalized to `#rrggbb` (alpha dropped) and a `subscribed` flag; the read set stores `color` per entry (`[{ url, name, enabled, color? }]`, validated on both sides) and `busy` tags every interval of a colored calendar `color`. On the app, `lib/calendarColors` makes the colors **distinct in read-set order** — iCloud's color when no earlier calendar took it, else the next unused entry of a ten-hue Apple-like palette — and joins untagged iCloud intervals to it by calendar name, so a proxy that tags no color still renders distinctly. Busy blocks mix the color into the recessed tint (16%) and inset ring (42%) and carry a dot beside the source tag; the busy popover and each picker row show the same dot. **The Outlook feed is one more calendar in the color sense (chunk 51c):** the Planner passes it to `calendarColorMap` as an extra entry once `/busy` reports it configured, colored after the read set under the same distinctness rule, keyed by `outlookColorKey(feedName ?? 'Outlook')` — namespaced, because an Outlook published calendar and iCloud's default are both commonly named "Calendar". An Outlook block keeps its 135° hatch (the *source* cue — stale semantics ride on it, 55% opacity unchanged) over the color tint (the *identity* cue); without a color (pre-fetch, unconfigured) the flat `--busy-outlook*` tokens stand. The picker lists the feed last as a read-only row (dot, feed name, `FEED` tag, no switch); the chip count stays `n/m` over iCloud. The proxy is untouched and never writes to Outlook. **Tight blocks (chunk 51c):** `PLANNER.hourH`/`mHourH` are unchanged (52/48 — the drop-slot math depends on them), so a 30-minute block is 24px on desktop and 22px on mobile; `BusyBlock` renders the title from 18px, and below 30px uses `py-[2px]`, 10px type, line-height 1.1. The 14px `blockPos` floor (15 minutes) stays text-free — the popover is the reveal. `TaskBlock` already shows its title at 24px (its `tight < 40` mode has no title threshold) and was left alone. The Dashboard busy strip is text and unchanged.
...
- Page margins (chunks 51d/51e): the shell fills the window — no max-width. Default padding is declared as `--shell-side` / `--shell-top` / `--shell-bottom` on `.shell` in `index.css` (16/28/40px sides by breakpoint, 24–32px top, 32px bottom; phones add the bottom-nav clearance). Settings → Display → Page margins opens `MarginEditor`: draggable guides on the content wrapper's padding edges, live preview, Save persists `hup:shellMargins` per device (localStorage, no schema, no sync) as an inline override of those variables; saving the defaults clears the key.

**Dashboard Today panel (chunk 41 merge, built on `main`):**
- The dashboard mounts a Today panel in one of three variants — `stacked`/`banner` full-width above the columns, `rail` beside them — or `off` (renders nothing), chosen in Settings → Display ("Today list layout"). Both the variant preference (`localStorage['hup:todayList']`, Zustand) and Today membership itself (a `Set<taskId>` seeded from `autoTodayIds` — P1 tasks plus `remindAt` due/overdue — and toggled via the sun button on panel + dashboard rows) are client-local: no schema, no sync. Membership persists per-device via day-scoped localStorage deltas (`hup:todayPlan`, chunk 43 — see §4 "Today membership"): manual pins and removals survive remounts and reloads within the same local day and reset at day rollover; the auto-derived portion re-derives on every seed/reconcile. Category/Subcategory drill-downs deliberately omit the sun toggle. `dueAt`/`settings.timezone`-aware derivation remains deferred (PROGRESS backlog); DB persistence was considered and rejected in chunk 43 (membership is a per-device view concept).
