...
**Exact counts are scored at the app's own mutation boundary, not from a browser (chunk 49, run `0831k`; supersedes the chunk-48 wording below).** Every browser instrument observes the *network*; these assertions are about what the app *issues*, and between the two sit a service worker (active on prod), retries, preflights and the proxy. `src/lib/plannerCalendarMirror.counts.test.ts` drives the real module with the real `calendarApi` through a stubbed `fetch` and reports a method-level log, deterministically: on `d075268` one `PATCH` per drag-move with none after the post-move refetch, one `DELETE` per unschedule, one orphan-sweep `DELETE` across deferred passes — each proven red against the build it targets. Run it in CI; use a live run only for layers *below* the app, and only with a server-side proxy log. **A network log belongs to the driver, not the environment:** `read_network_requests` exists on Claude in Chrome and the desktop built-in browser pane; the **Control Chrome MCP has none** (resource timing only — `responseStatus`, no `method`). **Name the driver in every run record's header.** Superseded wording follows.

**~~The driver's network log is the instrument for exact counts — and it does carry HTTP methods (chunk 48, runs `0831g`/`0831h`).~~** The chunk-43 note above says prod gives resource timing with "no HTTP method"; that premise is wrong. `read_network_requests` reports method and status on prod and does not duplicate. **Resource timing over-counts:** in both runs it reported 2 entries for a single `DELETE` where the netlog reported 1 (a fetch served through a service worker can surface more than one entry). Score exact-count assertions off the netlog; keep resource timing for ordering, for windows marked during a run, and for mount-cost reads. Two further driver facts: **request tracking resets on every reload** and must be re-armed after one, and **console tracking arms on first call** and cannot be asserted retroactively — call `read_console_messages` once immediately after the first load if console cleanliness is in scope.

**A reload between a placement's proxy POST and `stampUid` orphans the event (chunk 48, D-2).** The page unloads before the uid is written to the block, so the event exists with nothing claiming it and the post-reload backfill creates a second one. Confirm the block's `calendar_uid` is non-null in the DB before reloading after a placement — or force the `/busy` refresh with Force resync, which has no such window. Self-healing either way: the orphan sweep removes the stray on a later load.
