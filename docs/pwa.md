...
The service worker is generated by `vite-plugin-pwa` with the default
`generateSW` strategy (Workbox). Two cache layers are in play:
...
2. **Runtime cache for Supabase REST + GraphQL** — `NetworkFirst` strategy
   keyed on `*.supabase.co/rest/v1/*` and `*.supabase.co/graphql/v1/*`.
   30-second network timeout, 5-minute freshness window, capped at 200
   entries. Writes (POST/PUT/PATCH/DELETE) are never cached — Workbox only
...
## Notifications (forward reference — chunk 14)
...
(60-second polling while a tab is open) is the only delivery path. The PWA
shell landing in this chunk is the prerequisite for that flow.
