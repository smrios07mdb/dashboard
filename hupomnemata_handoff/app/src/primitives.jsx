...
// Custom checkbox — square, accent fill when checked. Springs on check.
const Check = ({ checked, onChange, size = 18, label, indeterminate, style }) => {
  const boxRef = React.useRef(null);
  const mnt = React.useRef(false);
  const prev = React.useRef(checked);
  React.useEffect(() => {
    const was = prev.current; prev.current = checked;
    if (!mnt.current) { mnt.current = true; return; }
    if (checked && !was && boxRef.current) {
      boxRef.current.style.animation = 'none';
      void boxRef.current.offsetWidth;
      boxRef.current.style.animation = 'checkPop .34s cubic-bezier(.2,.8,.3,1.4)';
    }
  }, [checked]);
  return (
  <label style={{ display: 'inline-flex', alignItems: 'center', gap: 10, cursor: 'pointer', ...style }}>
    <span ref={boxRef} aria-hidden style={{
