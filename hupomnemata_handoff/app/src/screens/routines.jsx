...
function DayGrid({ items, logs, routine, accent }) {
  const today = new Date();
  const cells = [];
  for (let d = 13; d >= 0; d--) {
    const date = new Date(today); date.setDate(today.getDate() - d);
      ...
      {cells.map((c, i) => (
        <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <span style={{
            ...
            background:
              c.state === 'full' ? accent :
              c.state === 'partial' ? `color-mix(in srgb, ${accent} 28%, transparent)` :
              c.state === 'faded' ? 'transparent' : 'var(--bg-alt)',
            boxShadow: c.state === 'full' ? `0 2px 8px -2px ${accent}` : 'none',
...
function RoutinePanel({ routine, items, logs, todayKey, onToggle, onAdd, onRename, onRemove, onReorder, editing, setEditing }) {
  ...
  const accent = routine === 'morning' ? 'var(--jewel-gold)' : 'var(--jewel-amethyst)';
  ...
  return (
    <section style={{
      ...
      borderRadius: 'var(--radius-md)', padding: 22, boxShadow: 'var(--shadow-md)',
    }}>
      <header style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
        ...
        <div style={{
          ...
          background: streak > 0 ? accentSoft : 'var(--bg-alt)',
          color: streak > 0 ? accent : 'var(--ink-3)',
          ...
          {streak > 0 ? (
            <React.Fragment>
              <IFlame s={12} style={{ color: accent }}/>
              <span className="num" style={{ color: accent, fontSize: 12, fontWeight: 700 }}>{streak}</span>
      ...
      <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--line)' }}>
        ...
        <DayGrid items={items} logs={logs} routine={routine} accent={accent}/>
      </div>
    </section>
  );
}
