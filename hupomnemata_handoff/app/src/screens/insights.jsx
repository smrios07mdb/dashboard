...
function pickPalette(subs, categories) {
  ...
  const workShades = ['#2f9b73', '#3aa885', '#1f8f6b', '#2f9aa6', '#4cb892', '#1d7d5e', '#3bb0a0'];
  const persShades = ['#df6240', '#d24b8c', '#cf5d39', '#c9942f', '#e07a55', '#b84e6e', '#d8825a'];
  const map = {};
  let w = 0, p = 0;
  subs.forEach(s => {
...
function Insights({ data, mobile }) {
  ...
  // real trend: second-half daily avg vs first-half
  const _half = Math.floor(series.length / 2);
  const _sum = (arr) => arr.reduce((s, r) => s + subs.reduce((a, sb) => a + (r[sb.id] || 0), 0), 0);
  const _firstAvg = _half ? _sum(series.slice(0, _half)) / _half : 0;
  const _lastAvg = (series.length - _half) ? _sum(series.slice(_half)) / (series.length - _half) : 0;
  const trend = _firstAvg ? Math.round((_lastAvg - _firstAvg) / _firstAvg * 100) : 0;

  return (
    <div className="screen">
      <header style={{ display: 'flex', alignItems: 'baseline', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
        <h1 className="display" style={{ margin: 0, fontSize: 40, fontWeight: 500, letterSpacing: '-.02em', color: 'var(--ink)' }}>
          Insights
        </h1>
        <span className="label" style={{ paddingBottom: 6 }}>Where the time went</span>
        {totalMinutes > 0 && (
          <span style={{
            marginLeft: 'auto', alignSelf: 'center',
            display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 11px',
            borderRadius: 999, fontSize: 12, fontWeight: 600,
            background: trend >= 0 ? 'var(--work-soft)' : 'var(--bg-alt)',
            color: trend >= 0 ? 'var(--work)' : 'var(--ink-2)',
            border: '1px solid ' + (trend >= 0 ? 'color-mix(in srgb, var(--work) 30%, transparent)' : 'var(--line)'),
          }}>
            <span style={{ fontSize: 13 }}>{trend >= 0 ? '▲' : '▼'}</span>
            <span className="num">{trend >= 0 ? '+' : ''}{trend}%</span>
            <span style={{ color: 'var(--ink-3)', fontWeight: 500 }}>vs first half</span>
          </span>
        )}
      </header>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 'var(--radius-md)', padding: '18px 20px', boxShadow: 'var(--shadow-md)' }}>
        {totalMinutes === 0 ? (
          <div style={{
