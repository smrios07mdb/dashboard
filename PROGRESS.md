...
Last updated: 2026-05-29 (chunk 14 notifications shipped — Web Push live end-to-end, on-device verify passed; cron + Edge Function active in Supabase project dctfspcbkqvvyptddtif)
...
| 11 | AI triage | dashboard | ☑ | Claude Code | 3a38c2f | — | Final landed model: `claude-haiku-4-5` (verified current on docs.claude.com — alias for `claude-haiku-4-5-20251001`). Model cycle in this chunk's commits: `claude-sonnet-4-5` (pre-chunk planning) → `claude-sonnet-4-6` (3a38c2f, 4-5 superseded) → `claude-haiku-4-5` (4f67aa9, sufficient for this trivial classification task and cheaper per call). `repo.settings.get/update` take a `userId` arg (brief implied no-arg) — handled via `useSession` + `supabase.auth.getSession()`, no `repo.ts` change; minimal Start highlight contained to `SubcategoryView` (`TaskRow` untouched); `timezone` included in AI payload (superset of ARCH §10); `design/Settings.tsx` mock followed the swap to `claude-haiku-4-5` in 4f67aa9 (originally framed as a frozen visual reference). · Smoke pass 2026-05-28 via Cowork Chrome MCP against deployed PWA at `3a38c2f` (gh-pages `79db2cb`): **6/6 PASS**. Test 4 (missing-key CTA): copy "You haven't added an Anthropic API key yet." + "Add your API key in Settings" button. Test 5 (invalid-key 401): saved `sk-ant-...INVALID...XXX` → triage returned "API key rejected. Update it in Settings." + "Update key in Settings" button; network log confirmed POST `https://api.anthropic.com/v1/messages` returned `401`. Test 6 (DevTools direct-call verification): same network read confirmed call goes direct to `api.anthropic.com` with no proxy hop. Test 1 (save real key): "API key saved" toast; field re-loads from Supabase on next Settings nav (RLS-scoped persistence working). Test 2 (30 min window): 2 cards (Drop off dry cleaning 20m, Reply to Q2 planning thread 15m) + note "Both tasks together total exactly 35 minutes, just slightly over your budget…" — card format matches spec (title / `CATEGORY · SUBCATEGORY` / estimate / due / one-sentence reason / Start button). Test 3 (vary minutes): 5m → 1 card with prep-only note "no task fits cleanly — use this time to review and mentally queue…"; 60m → 3 cards (50m total, note mentions "category balance" — one of the system-prompt criteria); 120m → 3 cards (95m total, swaps in 1h "Draft auth migration spec" which doesn't fit the smaller windows). Composition genuinely shifts with available time. Bonus: Start button on a 120m result navigated to `/subcategory/<id>?task=<id>`, target row highlighted on landing — satisfies the chunk-11 "Start navigates correctly" acceptance and the review-notes "minimal Start highlight contained to `SubcategoryView`" detail. Observation (not a fail): AI's reasoning text mentions dates ("May 30", "May 31") one day off from card-displayed due dates ("May 29", "May 30") on some runs — LLM date-handling quirk, not a UI bug; `timezone` is already in the payload per ARCH §10. Mobile-bottom-sheet variant + malformed-JSON error branch not exercised in this pass (no easy way to force a malformed response from a live API; mobile viewport not in the smoke spec). |
| 12 | CalDAV proxy | dashboard-caldav-proxy | ☑ | Claude Code | e5f6b26 | — | Build in separate `dashboard-caldav-proxy` repo. SHA `e5f6b26` = root commit, 25 files, 42 unit tests green. Deployed to Vercel `main` at https://dashboard-caldav-proxy.vercel.app; `/api/health` returns `{ok:true}` (post-deploy verified 2026-05-28). Stack: Node 24.x runtime, `tsdav` ^2.2.2, `jose` ^6.2.3, `zod` ^4.4.3, `@supabase/supabase-js` ^2.106.2. **JWT verification:** Supabase JWKS publishes asymmetric ES256/P-256 (pre-deploy probe confirmed per canon bundle resolution #5); proxy uses `jose.createRemoteJWKSet` + `jwtVerify` with `algorithms: ['ES256']`, no HS256 fallback. **Credential transport:** `settings.caldav_app_password_encrypted` is `bytea` per ARCH §4, but supabase-js is JSON-over-HTTP and can't carry raw bytes — encoded as Postgres `\x`-hex text format on both legs (`bufferToBytea` / `byteaToBuffer` in `api/_lib/supabase.ts`); `crypto.ts` (AES-GCM encrypt/decrypt) stays a pure Buffer↔string. See Decisions log 2026-05-28 (chunk 12). **CORS:** host-only exact-match allowlist via `ALLOWED_ORIGINS`, never `*`; production set to `https://smrios07mdb.github.io,http://localhost:5173` (no whitespace around comma — parser doesn't trim). **Handler signature:** `export default { fetch }` Web-standard form on Vercel's Node runtime (first-deploy verified). **Shared layer:** `api/_lib/http.ts` wraps CORS + JSON envelope + auth/error handling — justified addition beyond the prompt's literal file list, keeps security-relevant CORS logic single-sourced across 4 endpoints. **Error classification:** `classifyError` (in `api/_lib/caldav.ts`) returns `kind: 'auth'` only for unambiguous 401/Unauthorized; 403/429/5xx → `other`; transport-level (fetch failed, ECONNREFUSED, ENOTFOUND, ETIMEDOUT, EAI_AGAIN) → `network`. Rationale captured in code comments: "false-positive `auth` is worse than false-negative" because only `auth` flips `caldav_status='auth_failed'` and triggers the §7 reconnect banner. **Two first-deploy unknowns resolved cleanly:** `api/_lib/*` correctly excluded from Vercel routing (underscore convention held), `export default { fetch }` accepted by Node runtime — both load-bearing. **Live iCloud acceptance criteria** (test-credentials returns calendars; save-credentials writes encrypted bytea; events lands in Calendar.app; 401 without JWT; iCloud 401 → `caldav_status='auth_failed'`) remain operator-verified post-deploy via the README's curl block — no live iCloud creds at build time. Clean-room `npm install` from empty state resolved without version downgrades; `npm audit --omit=dev` reports 0 vulnerabilities on the deployed runtime path (audit warnings sit entirely in the `vercel` CLI devDep tree, not shipped). · **Live-verification follow-ups (2026-05-28 operator smoke).** Both "first-deploy assumptions" the build-time review notes called out came back as bugs during live verification, plus two more not anticipated. Four post-chunk fix commits landed on `dashboard-caldav-proxy/main`: `be8b4cd` (relative `_lib/*` imports needed `.js` extensions per Node 24 native ESM resolution; vitest's auto-extension resolution hid this in tests — `Cannot find module '/var/task/api/_lib/caldav'` in prod logs), `da7fca7` (tsdav 2.2.2 ships a broken ESM entry: `dist/tsdav.esm.js` is ESM syntax but tsdav's package.json lacks `"type": "module"` and the file isn't `.mjs` — Node refused ESM load, fell back to CJS parse, threw SyntaxError on `import`; loaded via `createRequire` to force the `require` condition → `dist/tsdav.cjs.js`), `9f1300f` (extracted tsdav loading to `api/_lib/tsdav.ts` wrapper so vitest's `vi.mock` re-intercepts — runtime-constructed require()s aren't hooked by vitest's loader, but a normal ES module wrapper is). Fourth issue had no commit — Vercel-side fix only: three production env vars (`SUPABASE_URL`, `SUPABASE_JWKS_URL`, `ALLOWED_ORIGINS`) were stored as literal empty strings despite `vercel env ls` showing "Encrypted"; surfaced as generic 401 `{ok:false,error:"unauthorized"}` from `requireUser` (because `new URL("")` throws → wrapper catches → maps to unauthorized). Cost ~90 min before `vercel env pull` + grep surfaced the empties. Re-set via `vercel env rm` + `echo -n "value" \| vercel env add` from CLI. **Deploy posture:** Hobby-tier git-integrated auto-deploys block on "commit author does not have contributing access" message even when author email = Vercel account email (cause unclear). CLI deploys (`vercel --prod` or `vercel deploy --prebuilt --prod`) bypass cleanly and are the canonical path until upgrade. **Live verification outcome (operator smoke, real Apple ID + Supabase JWT):** `/api/health` 200; no JWT → 401; valid JWT + real app-specific password → calendars list (4 calendars returned); `save-credentials` writes encrypted bytea to `settings` (verified `ok:true`); `events` creates VEVENT and "Chunk-12 smoke test" event appeared in Calendar.app within seconds (uid returned); deliberately-bad password → next `busy` call returns `{ok:false,error:"auth_failed"}` and flips `caldav_status='auth_failed'`; good creds re-saved as cleanup. **Brief sidecar resolution #7 ("bytea pass-through") was wrong** — refined via 2026-05-28 (chunk 12) decisions-log entry below for `\x`-hex transport. **Brief resolution #5 (JWKS pre-check)** held — Supabase JWKS publishes `ES256/P-256` with kid `8acf5fc8-3d33-4011-99ae-c361e92942ac`, matching the JWT signing key end-to-end. |
| 13 | Calendar integration + reconnect UX | dashboard | ☑ | Claude Code | 1c550a0 | — | Chunk-13 implementation. Single commit `1c550a0` on `chunk-13-calendar-integration`, fast-forward merged to `main` at `eaa0ae0`, pushed; GitHub Actions "Deploy to GitHub Pages" run #55 (42s) + "pages build and deployment" #23 (31s) both green on 2026-05-29. Deploy URL: https://smrios07mdb.github.io/dashboard/. 18 files changed / 1769 insertions; 12 net-new source files including `src/components/BusyStrip.tsx`, `src/components/BlockTimeSheet.tsx`, `src/components/ReconnectBanner.tsx`, `src/lib/calendarApi.ts`, `src/lib/slots.ts` (+ tests), `src/lib/busyCache.ts`, `src/lib/session.ts` (+ tests), shadcn `alert` and `badge` primitives, plus `docs/calendar.md`. Modified surface: `src/screens/Settings.tsx` (massive — 311-line calendar integration block added with Apple ID field, app-password input, Test connection / Save / Disconnect controls, connection status badge with `caldav:lastVerifiedAt` localStorage timestamp), `src/components/TaskMenu.tsx` (+14 lines: new "Block time" menuitem between "Set reminder…" and "Edit notes"), `src/screens/Dashboard.tsx` (+2: BusyStrip mounted above the day header), Dexie schema + types extended for cached busy intervals. **Tests:** 114/114 green at merge time, `tsc --noEmit` clean. **Live cycle via Cowork Chrome MCP against deployed PWA 2026-05-29 (real Apple ID + Supabase session):** ✅ BusyStrip renders correctly with empty-state copy `BUSY · No busy times today` when proxy `/api/calendar/busy` returns `[]`. ✅ Settings → Calendar connection shows green `Connected · verified just now` badge with Apple ID `srios@smu.edu` populated from saved settings; Test connection / Save / Disconnect / app-password input all wired (Test connection button is correctly disabled when password field is empty since saved creds live on proxy not client). ✅ TaskMenu's new "Block time" item opens BlockTimeSheet as a right-side panel. ✅ BlockTimeSheet's slot computation (`src/lib/slots.ts`) generated three overlapping 20m windows for the 20m "Drop off dry cleaning" task (12:15–12:35, 12:30–12:50, 12:45–1:05 PM EDT) — gives the user options rather than picking one "optimal" slot (decisions log entry below). ✅ Slot selection toggles `Selected` badge; "Add to Apple Calendar" fires `POST /api/calendar/events` to proxy (network log captured the OPTIONS 204 preflight + the POST), toast surfaces `Event added to Apple Calendar`, real event lands in operator's iCloud Calendar.app (operator-confirmed on iPhone — the "Drop off dry cleaning" 12:15–12:35 PM event appeared within seconds). **ReconnectBanner not exercised live** (would require deliberately saving a bad password to flip `caldav_status='auth_failed'` — operator opted to skip since the component renders deterministically in code review and the auth-failed flip path was already verified end-to-end during chunk-12 brief criterion #6 on 2026-05-28). **Contrast with chunk 12:** zero post-chunk follow-up bugs surfaced during the live cycle — the chunk-12 round of `.js`-extension / tsdav-ESM / Vercel-env-var fixes had already paved the proxy side, so chunk-13's client work landed cleanly. **VITE_CALDAV_PROXY_URL wiring** (set during chunk-12 close-out in `.env.local` + GH Actions secret on 2026-05-28) verified working — Pages build embedded the URL correctly, browser-side calls route to `https://dashboard-caldav-proxy.vercel.app`. Branch `chunk-13-calendar-integration` is now `eaa0ae0`-ancestor and can be deleted or recycled for hotfix work. |
| 14 | Notifications + race-safe `notified` flag | dashboard + supabase | ☑ | Claude Code | 48637e8 | — | Chunk-14 implementation. Branch `chunk-14-notifications` tip `b6a047d` squash-merged to `main` as `48637e8` (PR #1, "feat(notifications): chunk 14 — Web Push + race-safe notified flag (ARCH §9)"). 20 files changed / +1364 / -32; net-new: `src/sw.ts` (custom SW under `injectManifest`), `src/lib/push.ts` + tests, `src/lib/pushPayload.ts` + tests, `src/lib/inAppReminders.ts` + tests, `src/components/InAppReminders.tsx`, `supabase/functions/notify-due-reminders/index.ts` + `deno.json`, `supabase/migrations/06_claim_due_reminders.sql`, `supabase/migrations/07_notify_cron.sql` (inert template), `scripts/gen-vapid.ts`, `docs/notifications.md`. Modified surface: `vite.config.ts` (generateSW → injectManifest), `src/App.tsx` (+RealtimeBridge + InAppReminders mount), `src/db/repo.ts` (push_subscriptions table), `src/screens/Settings.tsx` (+150 lines: notifications panel — enable/disable, permission state, iOS-gate copy, debug "send test" affordance), `tsconfig.app.json`, `docs/pwa.md`. **Tests:** all green at merge; `tsc --noEmit` clean. **SW Workbox spot-check (pre-merge gate, Cowork 2026-05-29):** new `src/sw.ts` reproduces chunk-4's PWA contract verbatim — `precacheAndRoute(self.__WB_MANIFEST)` + `cleanupOutdatedCaches()`, `NavigationRoute` fallback to `/dashboard/index.html`, Supabase REST + GraphQL stays `NetworkOnly` (Bug-B locked subsystem), `skipWaiting()` + `clientsClaim()` for autoUpdate semantics. Fonts continue via bundled `@fontsource` woff2 in the precache glob (no Google-Fonts runtime route needed). **Go-live sequence (Cowork-driven, 2026-05-29):** VAPID keypair generated via `node scripts/gen-vapid.ts`; public key placed in `.env.local` and `VITE_VAPID_PUBLIC_KEY` GH Actions secret (operator); `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` set via `supabase secrets set` (operator-entered, never echoed). Migration `06_claim_due_reminders.sql` applied via Supabase SQL editor (surgical, see Deviations); Edge Function deployed with `supabase functions deploy notify-due-reminders --no-verify-jwt` (image v1.74.0 pulled). Cron scheduled via Dashboard → Integrations → Cron UI (Option B in `07_notify_cron.sql`): name `notify-due-reminders`, schedule `* * * * *`, type Supabase Edge Function, POST, timeout 1000ms. **Side effects enabled during cron setup:** `pg_cron` (schema `pg_catalog`) and `pg_net` (schema `extensions`) — both flagged as required by `07_notify_cron.sql` header; recorded in decisions log. **On-device verify (operator, 2026-05-29):** iPhone (16.4+), PWA installed to Home Screen, notifications permission granted, task created with ~90s reminder, app force-closed, push arrived on lock screen, tap focused the PWA at the task URL — single-device verify passed; race check across two devices not exercised. **Deviations from prompt:** (1) `supabase db push` failed with "Remote migration versions not found in local migrations directory" (remote had a `20260524215827`-timestamped row in `supabase_migrations.schema_migrations` not present in `supabase/migrations/`); used the spec's surgical fallback — pasted `06_claim_due_reminders.sql` into the SQL editor instead — and skipped `migration repair` to avoid mutating remote tracking state. (2) `supabase secrets set` rejected the macOS Keychain-stored access token with "Invalid access token format. Must be like `sbp_0102...1920`." despite `supabase login` + `supabase link --project-ref dctfspcbkqvvyptddtif` having succeeded in the same shell against the same keychain entry. `--debug` produced no additional output. Worked around by generating a personal access token at `https://supabase.com/dashboard/account/tokens` and `export SUPABASE_ACCESS_TOKEN=sbp_...`; secrets set succeeded immediately. Token rotated after the session. (3) PROGRESS.md row 14 originally referenced commit `b6a047d` (branch tip); after squash-merge the canonical SHA on main is `48637e8` and is what's recorded above. |
...
| 2026-05-28 | Swapped AI triage model from claude-sonnet-4-6 to claude-haiku-4-5 — sufficient for this trivial classification task and cheaper per call. ARCH §2/§10, chunk-11 prompt, `src/lib/ai.ts`, `ai.test.ts` assertion, Settings note, and the `design/Settings.tsx` mock all updated (commit 4f67aa9). |
| 2026-05-28 | (chunk 12) Encrypted iCloud app-specific password transports over supabase-js as Postgres `\x`-hex bytea text format, not as driver-level binary. Reason: supabase-js is JSON-over-HTTP via PostgREST — raw bytes can't traverse. `api/_lib/supabase.ts` exposes `bufferToBytea` / `byteaToBuffer` helpers; `api/_lib/crypto.ts` (AES-GCM encrypt/decrypt) stays a pure Buffer↔string interface. Column type is still `bytea` per ARCH §4 — the encoding is a transport concern, not a schema change. Refines the chunk-12 brief sidecar note that framed bytea as "pass bytes through." Pattern applies to any future server-side encrypted-blob columns accessed via supabase-js / PostgREST. |
| 2026-05-28 | (chunk 12) Server-side npm packages with broken dual-CJS/ESM packaging — ESM entry is `.js` syntax but the package's own package.json lacks `"type": "module"`, so Node refuses ESM load — are loaded via a thin `api/_lib/<pkg>.ts` wrapper that uses `createRequire(import.meta.url)` to force CJS resolution through the package's `require` condition. Wrapper does `require('<pkg>')` and re-exports named functions as ESM. Two benefits: (a) sidesteps the broken ESM entry by routing to the working CJS bundle, (b) makes vitest's `vi.mock` intercept correctly — runtime-constructed `require()` calls aren't hooked by vitest's loader, but a normal ES-module wrapper is. First applied to `tsdav` 2.2.2 (`api/_lib/tsdav.ts`). Apply the same pattern to any future server-side npm dep that exhibits this packaging issue. |
| 2026-05-28 | (chunk 12) CalDAV proxy deploys via Vercel CLI (`vercel --prod` from `~/Documents/dashboard-caldav-proxy`), not Vercel's git-integrated auto-deploy. Reason: Hobby tier rejects git-push auto-deploys with "commit author does not have contributing access" even when commit-author email matches the Vercel account email; CLI deploys bypass the check. `vercel deploy --prebuilt --prod` after `vercel build --prod` is the most predictable variant — uploads `.vercel/output/` verbatim, no remote rebuild, no cache. Until an upgrade off Hobby, do not rely on `git push` to deploy the proxy. |
| 2026-05-29 | (chunk 13) BlockTimeSheet's slot picker returns multiple **overlapping** free-time suggestions for a single task rather than a single "optimal" slot. For a 20m task on a day with no busy intervals, `src/lib/slots.ts` produced three 20m windows offset by 15m steps (12:15–12:35, 12:30–12:50, 12:45–1:05 PM) — the user picks which feels right rather than the app committing on their behalf. Rationale: there's no single objective "best" 20m slot inside a free block; offering options preserves agency and reduces "the app decided wrong" friction. Constraint to preserve in future chunks that build on slot suggestion (rescheduling, drag-to-reschedule, bulk-block): keep N≥3 suggestions and let UI selection be explicit; don't auto-pick the head of the list. |
