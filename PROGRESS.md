...
Last updated: 2026-05-27 (chunk-10 smoke pass v3: 7/7 PASS; one v3 setup-gap finding worked around)
...
| 2026-05-27 | (chunk 10 smoke v3 setup gap) Wipe my data archives routine_items (sets `archived_at = now()`) rather than hard-deleting them, so rows from prior smoke runs remain "required" for today's streak math per ARCH §11 (`archivedAt >= startOfDay(today)` → required). Smoke v3 worked around this by backdating archived items' `archivedAt` to 2020-01-01 via `window.__claudeDashboard.repo.routineItems.update()` after each wipe. Real-fix candidates: (i) add a fourth Developer button "Hard-delete archived routine items" mirroring the Reset-routine-logs pattern (online-only, no outbox); (ii) change Wipe to hard-delete `routine_items` instead of archiving (changes prod UX); (iii) leave Wipe alone and document the archive-backdate as a permanent v3+ harness step. (i) is the cleanest — file as a Revisions chunk-6 or chunk-10 follow-up. Same gap exists for archived `subcategories` if the smoke surface ever extends there. |
