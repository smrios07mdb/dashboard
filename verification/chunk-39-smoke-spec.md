...
**Runs:** 2026-08-30 — `verification/chunk-39-smoke.md` (10/10 PASS, Sunday
variant, operator-waived §0.3).

**Amendment (2026-08-30, F2):** dev StrictMode double-fires the Planner's
mount busy effect (2 identical `/busy` per mount), so mount-time request
counts are only assertable in a production build. Wherever a check counts
`/busy` requests, count them on the next **week navigation** or Force resync
(exact = 1), not "on the next Planner visit" / at mount.

## Checks

| # | Action | Expected |
|---|---|---|
| 1 | **Toggle gating.** Settings → Calendars. (a) With Apple Calendar connected and ok. (b) Set `caldav_status='auth_failed'` via SQL, Force resync, revisit Settings. (c) Restore `ok`. Click **On**. | Row `Write planner blocks to Apple Calendar` with help line `Blocks you schedule on the Planner appear as events on the selected calendar. They are excluded from busy time so they aren't counted twice.`; `role="group"` of the same name with `On`/`Off` buttons. (a) both enabled, `Off` has `aria-pressed="true"`. (b) both disabled + `Connect Apple Calendar to enable.`. (c) click On → toast `Planner blocks will sync to Apple Calendar.`, `On` `aria-pressed="true"`, `settings.planner_writeout = true`, `uiStore.busyRefreshKey` +1 (exactly one `/busy` request on the next week navigation or Force resync — mount counts are dev-StrictMode-doubled, see the F2 amendment above). |
...
| 10 | **Disconnect clears the flag; cleanup.** Settings → `On` again (reconcile will re-sync the stale event), then Disconnect Apple Calendar → confirm. Reconnect (Test + Save; amendment 2026-08-30: an SQL restore of the §0.7 CalDAV values is the accepted equivalent — the untouched encrypted password decrypting on the next `/busy` proves the same substance, with no credential typed), then On. Unschedule every smoke block; delete the four tasks. | After Disconnect: `planner_writeout = false`, row disabled with the hint. After reconnect + On: the remaining blocks' events are current. Each Unschedule → `DELETE …/events` → `{ ok: true }`. End state: `scheduled_blocks` count = pre-smoke; `plannerEvents` = `[]`; Calendar.app shows no `Smoke W*` events; **nothing left on iCloud** — record the uids that passed through. Console 0 throughout. |
